...
def init():
    def get_type():
        ...
        for k, v in ranks.items():
            ranks[k] = sorted(reversed_ranks[v], reverse=True)

        if len(reversed_ranks[4])>0:
            return hand.Rank(hand.Rank.FOUR_OF_A_KIND, reversed_ranks[4][0])

        if len(reversed_ranks[3])>0:
            if len(reversed_ranks[3])>1:
                return hand.Rank(hand.Rank.FULL_HOUSE, reversed_ranks[3][0]*10+reversed_ranks[3][1])
            if reversed_ranks[2]>0:
                return hand.Rank(hand.Rank.FULL_HOUSE, reversed_ranks[3][0]*10+reversed_ranks[2][0])
        ...
        if flush_suit:
            ...
            if straight_rank_value:
                return hand.Rank(hand.Rank.STRAIGHT_FLUSH,straight_rank_value)
            return hand.Rank(hand.Rank.FLUSH,prod(flush_cards[:5]))
        ...
        if straight_rank_value:
            return hand.Rank(hand.Rank.FLUSH, straight_rank_value)

        if len(reversed_ranks[3])==1 & len(reversed_ranks[2])==0:
            left_ranks = [card for card in card_combo if card.Rank == reversed_ranks[3][0]]
            return hand.Rank(hand.Rank.THREE_OF_A_KIND,reversed_ranks[3][0]*10000+left_ranks[0]*100+left_ranks[1])
        ...
        if len(reversed_ranks[2]>1):
            ...
            return hand.Rank(hand.Rank.TWO_PAIRS,
        ...
        if len(reversed_ranks[2]==1):
            ...
            return hand.Rank(hand.Rank.ONE_PAIR,
                             reversed_ranks[2][0] * 1000000 + prod(left_ranks[:4]))

        return hand.Rank(hand.Rank.HIGH_CARD,prod(card_combo[:5]))

    for card_combo in combinations(all_cards, 7):
        product = 1
        ranks = {}
        suits = {}
        card_combo = sorted(card_combo,reverse=True)
        for c in card_combo:
            product *= c.rank
